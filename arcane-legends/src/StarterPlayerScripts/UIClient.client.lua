...
for i = 1, #Config.Eggs do
	...
	mkButton(cell, {
		...
		Text = "Ouvrir",
		OnClick = function()
			local ok, result = remotes.BuyEgg:InvokeServer(i)
...
local function refreshPets()
	...
	for i = 1, #snapshot.Pets do
		...
		mkButton(row, {
			...
			OnClick = function()
				local ok, message = remotes.SetPetEquipped:InvokeServer(pet.Uid, not pet.Equipped)
				if not ok then
...
local rebirthButton = mkButton(rebirthContent, {
	...
	OnClick = function()
		local ok, message = remotes.DoRebirth:InvokeServer()
		if not ok then
			showNotification(tostring(message), "erreur")
		end
	end,
